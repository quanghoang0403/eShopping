...
namespace eShopping.Payment.VNPay.Model
{
    public class VNPayRefundRequestModel
    {
        ...
        public string VnPayTransactionDate
        {
            get
            {
                if (TransDate == null) return "";
                TimeZoneInfo tzi = TimeZoneInfo.FindSystemTimeZoneById(TimeZoneConstants.AsiaStandardTimeCode);
                var vnpayTransDate = TimeZoneInfo.ConvertTimeFromUtc(TransDate.Value, tzi);

                return vnpayTransDate.VnPayFormatDate();
            }
        }
