...
const Card3: FC<Card3Props> = ({ className = 'h-full' }) => {
  return (
    <div className={`nc-Card3 relative flex flex-col-reverse sm:flex-row sm:items-center rounded-[40px] group ${className}`} data-nc-id="Card3">
      ...
      <div className={`block flex-shrink-0 sm:w-56 sm:ml-6 rounded-3xl overflow-hidden mb-5 sm:mb-0`}>
        <Link href={'/blog-detail'} className={`block w-full h-0 aspect-h-9 sm:aspect-h-16 aspect-w-16 `}>
          <NcImage alt="" fill src={_getImgRd()} containerClassName="absolute inset-0" sizes="(max-width: 768px) 100vw, 30vw" />
        </Link>
      </div>
    </div>
  )
