...
export default function OrderDetailHistoryModal (props) {
  ...
  const loadingList = async () => {
    // const response = await orderDataService.getOrderHistoryByOrderId(orderId)
    // if (response) {
          ...
          return (
            <Row className="modal-order-detail-time">
              <Row className="modal-order-detail-time-minute">
                {record?.createdTime.format(DateFormat.HH_MM)}
              </Row>
              <Row className="flash-sale-list-date">
                ...
                <Col span={18}>
                  <span>{record?.createdTime.format(DateFormat.DD_MM_YYYY)}</span>
                </Col>
              </Row>
            </Row>
          )
