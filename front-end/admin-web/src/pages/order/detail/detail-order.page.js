...
import { EditIcon, OrderHistoryIcon, ShipperIcon } from 'constants/icons.constants'
...
export default function OrderDetail (props) {
  ...
  const loadCustomer = (orderID, customerID) => {
    if (customerID) {
      const data = { orderID, customerID, type: 2 }
      reportDataService.getOrderDetailTransactionReportAsync(data).then((res) => {
          ...
          {record?.isCombo
              ...
              {record?.orderComboItem?.orderComboProductPriceItems?.map((item, index) => (
                <>
                  ...
                  {item?.orderItemOptions?.length !== 0
                    ? (
                        item?.orderItemOptions?.map((itemOption, index) => (
            ...
            <>
              {record?.orderItemOptions?.length !== 0
                ? (
                    record?.orderItemOptions?.map((item, index) => (
                  <Row key={index} className="mt-2 mb-2">
