...
import { NoSubMenuIcon } from "constants/icons.constants";
import { PatternConstants } from "constants/pattern.constants";
import { useState, forwardRef, useImperativeHandle } from "react";
import { randomGuid } from "utils/helpers";

export const FormAddMenuItemForEditComponent = forwardRef((props, ref) => {
  const {
    t,
  ...
  useImperativeHandle(ref, () => ({
    ...
    setDataForEdit(data) {
      if (data) {
        const { hyperlinkOption } = data;
        setMenuItem(data);
        setHyperlink(hyperlinkOption);
        if (hyperlinkOption === Hyperlink.SUB_MENU) {
          setChildMenu(data?.subMenu);
        } else {
          setChildMenu(null);
        }
  ...
  const pageData = {
    ...
    noSubMenu: t(
      "menuManagement.menuItem.noSubMenu",
  ...
  const onChangeHyperlinkOption = (id) => {
    if (hyperlink === Hyperlink.SUB_MENU) {
      let subMenuData = subMenus?.find((x) => x.id === id);
      setChildMenu(subMenuData);
    } else {
      setChildMenu(null);
    }
  };
          ...
          <>
            ...
            >
              {hyperlink === Hyperlink.URL ? (
                ...
                <FnbSelectSingle
                  onChange={onChangeHyperlinkOption}
                  placeholder={placeholder}
                  showSearch
                  fixed
          ...
          <Col sm={24} lg={16} className="w-100">
            ...
            <Row className="justify-content-center">
              ...
              <Button type="primary" onClick={onSubmitForm}>
                {isEdit ? pageData.btnUpdate : pageData.btnAdd}
              </Button>
            </Row>
          </Col>
          <Col span={0} lg={8} className="w-100">
            <div className="child-menu-container">
              <div className="title-box">
                <h3 className="title-text">Child menu</h3>
              </div>
              <div className="child-menu-box">
                {childMenu && childMenu.onlineStoreMenuItems.length > 0 ? (
                  <>
                    <div className="child-menu-item">
                      <span title={childMenu.name} className="child-menu-name">
                        {childMenu.name}
                      </span>
                    </div>
                    {childMenu.onlineStoreMenuItems?.map((item) => {
                      return (
                        <div className="child-menu-item">
                          <span
                            title={item.name}
                            className="child-menu-item-name"
                          >
                            {item.name}
                          </span>
                        </div>
                  ...
                  <>
                    ...
                    <div className="text-info-box">
                      <span className="text-info">{pageData.noSubMenu}</span>
                    </div>
                  </>
                )}
              </div>
