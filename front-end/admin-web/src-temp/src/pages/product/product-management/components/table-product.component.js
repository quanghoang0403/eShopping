...
export default function TableProduct(props) {
  ...
  const tableSettings = {
    ...
    columns: [
      ...
      },
      {
        ...
        width: "15%",
        render: (_, record) => {
          const platformNames = record?.platforms?.map((p) => p.name);
          return (
            <Row>
              <Col span={24}>
          ...
          return (
            <div className="action-column">
              <EditButtonComponent
                ...
                onClick={() => onEditItem(record?.id, record?.name)}
  ...
  const onEditItem = (productId, productName) => {
    productDataService.getAllOrderNotCompletedByProductIdAsync(productId).then((res) => {
      const { preventDeleteProduct } = res;
              ...
              {onPopover ? (
                ...
                <>
                  <Col span={12} className="price-box-inline">
                    <span className="float-left mt-1" title={p?.priceName}>{p?.priceName}</span>
                  </Col>
