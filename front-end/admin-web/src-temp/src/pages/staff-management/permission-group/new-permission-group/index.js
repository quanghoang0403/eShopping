...
import { capitalize, sentenceToCamelCase } from "utils/index";
import "../index.scss";
...
export default function NewPermissionGroup(props) {
  ...
  const allowedPermissionIdsOnlineStore = [
    PermissionKeys.VIEW_THEME_STORE.toLowerCase(), 
    PermissionKeys.EDIT_THEME.toLowerCase(), 
    PermissionKeys.REMOVE_THEME.toLowerCase(),
    PermissionKeys.VIEW_BLOG.toLowerCase(),
    PermissionKeys.CREATE_BLOG.toLowerCase(),
    PermissionKeys.EDIT_BLOG.toLowerCase(),
    PermissionKeys.DELETE_BLOG.toLowerCase(),
  ];
  useEffect(() => {
    fetchData();
  }, []);
  ...
  const mappingPermissionGroups = (permissionGroups) => {
    return permissionGroups?.map((permission, index) => {
      const childrens = permission.permissions?.filter(x => x.permissionGroupId.toLowerCase() !== PermissionKeys.EDIT_COMBO.toLowerCase() ||
            allowedPermissionIdsOnlineStore.includes(x.id)).map((permission) => {
          ...
          <div className="float-right">
            <Checkbox checked={checked} onChange={onChangeCheckboxFullPermissions}>
              <div className="body-2">
                {pageData.textFullPermission} {t(`permissionsGroup.${permissionGroupTranslateKey}`)}
              </div>
            </Checkbox>
          </div>
        </Col>
