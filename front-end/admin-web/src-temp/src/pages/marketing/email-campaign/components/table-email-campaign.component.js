...
export default function TableEmailCampaign(props) {
        ...
        render: (_, record) => {
          switch (record?.status) {
            ...
            case EmailCampaignStatus.Finished:
              return <div className="status-finished">{t("marketing.emailCampaign.status.finished")}</div>;
            default:
              return <div className="status-active">{t("marketing.emailCampaign.status.active")}</div>;
          }
        },
      },
    ];
          ...
          return (
            <div className="qr-action-column">
              {hasPermission(PermissionKeys.EDIT_EMAIL_CAMPAIGN) && status === EmailCampaignStatus.Scheduled && (
                <EditFill className="icon-svg-hover pointer" onClick={() => onEditItem(id)} />
              )}
  ...
  const onEditItem = (id) => {
    history.push(`/email-campaign/edit/${id}`);
  };
