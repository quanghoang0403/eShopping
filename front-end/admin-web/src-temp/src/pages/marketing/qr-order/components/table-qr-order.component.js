...
import { CloneButtonComponent } from "components/clone-button";
...
import { formatDate, hasPermission, tryJsonString } from "utils/helpers";
          ...
          return (
            <div className="text-line-clamp-2">
              <Tooltip title={record.eventName}>
                <Link to={`/marketing/qr-order/detail/${record?.id}`}>{record.eventName}</Link>
              </Tooltip>
            </div>
          );
        },
    ...
    if (
      ...
      const actionColumn = {
        ...
        render: (_, qrCode) => {
          ...
          if (isStopped === true) {
            return <CloneButtonComponent onClick={() => onCloneQrCode(id)} />;
          }
          ...
          return (
            <div className="qr-action-column">
              ...
              {hasPermission(PermissionKeys.CREATE_QR_CODE) && (
                <CloneButtonComponent onClick={() => onCloneQrCode(id)} />
              )}
            </div>
          );
        },
