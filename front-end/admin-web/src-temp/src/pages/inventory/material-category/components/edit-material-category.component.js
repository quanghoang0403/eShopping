...
export default function EditMaterialCategoryComponent(props) {
  ...
  const [isSelectedAllMaterials, setIsSelectedAllMaterials] = useState(true);
    ...
    return (
      <Form form={form} name="basic" onFinish={onFinish} autoComplete="off">
        ...
        <div className="material-check-box-select-all-branch">
          <Checkbox onChange={(event) => onChangeOption(event)} checked={isSelectedAllMaterials}>
              {pageData.allMaterials}
          </Checkbox>
        </div>
        <Form.Item
          hidden={isSelectedAllMaterials}
        >
  ...
  return (
    <FnbModal
      width={"800px"}
      title={title}
      className="edit-material-category"
