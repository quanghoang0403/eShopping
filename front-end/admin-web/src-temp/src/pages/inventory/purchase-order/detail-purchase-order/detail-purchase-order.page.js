...
import { Button, Card, Col, Dropdown, Menu, Row, Space, Steps, Typography, message } from "antd";
import CancelConfirmComponent from "components/cancel-confirm/cancel-confirm.component";
import { FnbTable } from "components/fnb-table/fnb-table";
import { EditIcon, PrintIcon } from "constants/icons.constants";
...
import { formatNumberDecimalOrInteger, getCurrencyWithSymbol, hasPermission } from "utils/helpers";
import "./index.scss";
const { Step } = Steps;
...
export default function DetailPurchaseOrder(props) {
  ...
  const renderDropdownLink = () => {
    ...
    return (
      <Dropdown className="ant-dropdown-link button-action" trigger={["click"]} overlay={menu}>
        <a>
    ...
    <>
      ...
      <Row>
        ...
        <Col xs={24} lg={8}>
          <div className="steps-custom">
            <Steps
              className="fnb-steps fnb-steps-customize"
              size="small"
              progressDot
              ...
              responsive={false}
            >
              <Step title={pageData.status.order} />
      ...
      <div className="card-parent">
        <Card className="mt-3 mb-4 card-general-info" title={pageData.generalInfo}>
          <Row className="mb-3">
            <Col xs={24} lg={12}>
              ...
              <p className="mt-2">
                <Link to={`/inventory/supplier/${purchaseOrderDetail.supplier?.id}`} target="_blank">
            ...
            <Col xs={24} lg={12}>
              ...
              <p className="mt-2">
                <span className="ml-2">{purchaseOrderDetail.storeBranch?.name}</span>
              </p>
            </Col>
          </Row>
          <Row>
        ...
        <Card className="mt-1 card-material-info" title={pageData.materialInformation}>
          <FnbTable className="mt-4" dataSource={purchaseOrderMaterials} columns={columnsMaterial()} />
