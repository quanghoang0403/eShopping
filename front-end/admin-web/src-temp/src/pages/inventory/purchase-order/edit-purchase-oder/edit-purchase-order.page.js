...
export default function UpdatePurchaseOrderComponent(props) {
    ...
    if (match?.params?.id) {
      purchaseOrderDataService.getPurchaseOrderByIdAsync(match?.params?.id).then((res) => {
        if (res && res.purchaseOrder) {
          ...
          fetchInitData(res.purchaseOrder.purchaseOrderMaterials);
  ...
  const fetchInitData = async (purchaseOrderMaterials) => {
  ...
  const onSelectMaterial = async (id, pageNumber, pageSize) => {
    ...
    let formSelectedMaterials = purchaseOrder.selectedMaterials;
