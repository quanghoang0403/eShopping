      ...
      >
          ...
          <Col span={24}>
            {infoMaterial.isOpenPurchaseOrder ? (
              <>
                ...
                ></div>
                <div className="table-notification">{renderRowByIndex(infoMaterial?.purchaseOrders)}</div>
              </>
            ) : (
              <p
                dangerouslySetInnerHTML={{
        ...
        {infoMaterial.isOpenPurchaseOrder ? (
          ...
          <Row className="modal-footer">
            ...
            <Button danger onClick={() => onDelete(infoMaterial?.material?.id, infoMaterial?.material?.name)}>
              {pageData.delete}
            </Button>
          </Row>
        )}
