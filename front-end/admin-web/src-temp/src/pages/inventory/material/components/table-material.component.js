...
import { formatNumberDecimalOrInteger, getCurrency, getThumbnailUrl, hasPermission } from "utils/helpers";
import "../index.scss";
...
export const TableMaterial = forwardRef((props, ref) => {
  ...
  const onDeleteItem = async (materialId) => {
    var response = await materialDataService.checkDeleteMaterialByIdAsync(materialId);
    if (response?.isInUse) {
  ...
  const getColumns = () => {
    const columns = [
...

      {
        ...
        hidden: !hasPermission(PermissionKeys.DELETE_MATERIAL) && !hasPermission(PermissionKeys.EDIT_MATERIAL),
      },
  ...
  const onSelectedRowKeysChange = (newSelectedRowKeys) => {
    setSelectedRowKeys(newSelectedRowKeys);
  };
  ...
  const exportMaterial = (storeId) => {
    ...
    }/api/material/export-material?languageCode=${languageCode}&&storeId=${storeId}&keySearch=${keySearch}
          ...
          <Row className="total-cost-amount-row">
            <div className="total-cost-amount-div">
              <Input
                ...
                value={`${formatNumberDecimalOrInteger(totalCostMaterials)} (${getCurrency()})`}
              />
            </div>
            <Col span={24} className="col-contain-table-material-management">
              <FnbTable
                className="mt-4 table-material-management"
                columns={getColumns()}
