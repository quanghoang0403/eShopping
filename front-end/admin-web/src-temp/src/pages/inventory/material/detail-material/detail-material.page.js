...
export default function MaterialDetail(props) {
  ...
  const pageNotFoundlLink = "/pageNotFound";
  ...
  const getInitDataAsync = async (id) => {
    ...
    if (res.material) {
      ...
      history.push(`${pageNotFoundlLink}`);
    }
  ...
  return (
    <>
      <Row className="fnb-row-page-header">
        ...
        <Col xs={20} sm={20} lg={14} className="fnb-form-item-btn">
          <ActionButtonGroup
            arrayButton={[
              {
                action: (
        ...
        <Card className="fnb-box custom-box material-view-card-margin">
          ...
          <div className="d-flex">
            <p className="material-view-cost-per-unit-text">
              <b>{`${formatTextNumber(costPerUnit)} ${currency} /${initData?.material?.unitName}`}</b>
            </p>
          ...
          <Row className="material-view-row-margin">
            ...
            <Col span={12}>
              <div className="d-flex">
                ...
                <Tooltip placement="topLeft" title={pageData.inventoryMinQuantityTooltip}>
