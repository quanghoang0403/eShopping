...
export default function ImportMaterial(props) {
  ...
  const pageData = {
    ...
    infoDataFail: {
      ...
      costPerUnit: t("material.import.formError.infoDataFail.costPerUnit"),
      skuInvalid: t("material.import.formError.infoDataFail.skuInvalid"),
    },
  ...
  const handleDownloadTemplate = () => {
    ...
    link.href = `${env.REACT_APP_ROOT_DOMAIN}/api/material/download-template-material?languageCode=${languageCode}&storeId=${storeId}`;
    link.click();
  };
  ...
  const getErrorMessage = (col) => {
    const listError = [
      ...
      {
        col: 16,
        message: pageData.infoDataFail.costPerUnit,
      },
      {
        col: 17,
        message: pageData.infoDataFail.skuInvalid,
      },
    ];
