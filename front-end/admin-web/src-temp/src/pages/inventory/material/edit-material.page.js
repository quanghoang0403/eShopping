...
import {
  DELAYED_TIME,
  inputNumberRangeOneTo999999999,
  MaximumNumber,
  MinimumNumber,
...
export default function EditMaterialPage(props) {
  ...
  const fetchInitData = () => {
    ...
    materialDataService.checkDeleteMaterialByIdAsync(id).then((responseData) => {
      if (responseData?.isInUse) {
        setListObjectLockMaterial(responseData);
      }
    });
  };
    ...
    materialDataService.getMaterialByIdAsync(id).then((res) => {
      if (res) {
        ...
        let formValue = {
          ...
          unitId: data?.unit?.id,
          sku: data?.sku,
          minQuantity: data?.minQuantity === 0 ? "" : data?.minQuantity,
                ...
                rules={[
                  {
                    ...
                    message: pageData.inventory.quantityValidateMessageMinimumOne,
                  },
                ]}
              >
                <InputNumber
  ...
  const handleOk = () => {
    ...
    handleDeleteItem(param.id, materialName);
  };
