...
import { formatDate, formatTextNumber, getCurrency, hasPermission, shortString } from "utils/helpers";
  ...
  const getColumnsFee = () => {
    const columnFees = [
      ...
      },
      {
        ...
        render: (_, record) => (
          <Tooltip placement="topLeft" title={shortString(record.name, 50)}>
            <a onClick={() => showDetailFeeModal(record.id)}>
              <span className="text-line-clamp-2 text-color-feeName">{record.name}</span>
            </a>
          </Tooltip>
        ),
      },
      {
        ...
        render: (_, record) => (
          <Tooltip placement="topLeft" title={shortString(record.description, 50)}>
            <span className="text-line-clamp-2 text-color-desc">{record?.description}</span>
          </Tooltip>
        ),
      },
      {
      ...
      },
      {
        ...
        dataIndex: "feeServingType",
        key: "feeServingType",
        width: "14%",
        render: (_, record) => {
          return (
            <>
              {record?.feeServingTypes.map((type) => (
                <Row>
                  <p>{t(OrderTypeConstantTranslate[orderTypeNames[type.orderServingType]])}</p>
                </Row>
              ))}
            </>
          );
            ...
            <>
              {record.startDate && (
                <Row>
                  ...
                  <Col span={16} className="fnb-start-date">
              ...
              {record.endDate && (
                <Row>
                  ...
                  <Col span={16} className="fnb-end-date">
        ...
        render: (_, record) => {
          switch (record?.status) {
            case FeeStatus.Schedule:
              return <div className="fee-status scheduled">{t("promotion.status.scheduled")}</div>;
            case FeeStatus.Active:
              return <div className="fee-status active">{t("promotion.status.active")}</div>;
            default:
              return <div className="fee-status finished">{t("promotion.status.finished")}</div>;
          }
        },
      },
    ];
