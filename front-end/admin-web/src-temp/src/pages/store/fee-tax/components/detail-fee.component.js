...
export default function FeeDetailComponent(props) {
            ...
            <Checkbox className="w-100" checked={feeDetail?.isAutoApplied} disabled>
              ...
              <Tooltip
                ...
                title={
                  <div>
                    <p>
                      This fee will be applied automatically when created the order. Otherwise, staff must select
                      manually
                    </p>
                  </div>
                }
  ...
  const renderActionButtons = () => {
    return (
      <>
        <button onClick={closeFeeDetailModal} className={buttonLeaveClasses()}>
          {isModalLeaveOnly ? pageData.leaveButton : pageData.cancelButton}
        </button>
