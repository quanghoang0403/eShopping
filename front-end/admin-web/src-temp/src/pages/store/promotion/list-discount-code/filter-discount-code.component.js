...
export default function FilterDiscountCode(props) {
  const [t] = useTranslation();
  const [form] = Form.useForm();
  const { branches, promotionTypeOptions, pageSize, keySearch, setDataFilter } = props;
  const defaultValue = "";
  ...
  const pageData = {
    filter: {
      ...
      codeType: t("discountCode.codeType"),
      multiTimeUsedCode: t("discountCode.multiTimeUsedCode"),
      oneTimeUsedCode: t("discountCode.oneTimeUsedCode"),
    },
  };
  ...
  const onApplyFilter = () => {
    ...
    if (!isFormValid) {
      ...
      props.fetchDataDiscountCode(1, pageSize, keySearch, formValue);
    }
  };
  ...
  const countFilterControl = (formValue) => {
    ...
    let countCodeType = formValue.codeType === "" || formValue.codeType === undefined ? 0 : 1;
  ...
  const onChangeCodeType = (id) => {
    setSelectedCodeType(id);
  };
  ...
  return (
    <Form form={form} onFieldsChange={onApplyFilter}>
      <Card className="discount-code-filter-scroll">
        <div className="discount-code-filter">
          <Card className="form-filter-discount-popover ">
            ...
            <Row style={{ alignItems: "center", marginTop: "-12px" }}>
              <div className="first-column" style={isMobileMode ? { marginBottom: "-5px" } : { marginBottom: "0px" }}>
                <span>{pageData.filter.platformTitle}</span>
              </div>
              <div className="second-column">
                <Form.Item name="platformId">
                  ...
                  >
                    <Radio.Button value={defaultValue}>
                      {selectedPlatform === "" && <CheckOutlined className="check-icon" />} {pageData.filter.status.all}
                    </Radio.Button>
                    {listPlatforms?.map((p, index) => {
                      return (
                        <Radio.Button value={p.id}>
                          {selectedPlatform === p.id && <CheckOutlined className="check-icon" />} {p.name}
                        </Radio.Button>
                      );
                    })}
                  </Radio.Group>
            ...
            <Row style={{ alignItems: "center", marginTop: "-12px" }}>
              <div className="first-column" style={isMobileMode ? { marginBottom: "-5px" } : { marginBottom: "0px" }}>
                <span>{pageData.filter.codeType}</span>
              </div>
              <div className="second-column">
                <Form.Item name="codeType">
                  <Radio.Group
                    ...
                    defaultValue={defaultValue}
                    buttonStyle="solid"
                    onChange={(e) => onChangeCodeType(e.target.value)}
                  >
                    <Radio.Button value={defaultValue}>
                      {selectedCodeType === "" && <CheckOutlined className="check-icon" />} {pageData.filter.status.all}
                    </Radio.Button>
                    <Radio.Button value={1}>
                      {selectedCodeType === 1 && <CheckOutlined className="check-icon" />}{" "}
                      {pageData.filter.oneTimeUsedCode}
                    </Radio.Button>
                    <Radio.Button value={2}>
                      {selectedCodeType === 2 && <CheckOutlined className="check-icon" />}
                      {pageData.filter.multiTimeUsedCode}
                    </Radio.Button>
                  </Radio.Group>
                </Form.Item>
              </div>
