...
export default function EditDiscountCode(props) {
  ...
  const [isLimitOneTimeUse, setIsLimitOneTimeUse] = useState(false);
  ...
  const [isClickCustomerSegment, setIsClickCustomerSegment] = useState(false);
  ...
  const pageData = {
    ...
    edit: t("discountCode.btnEditDiscountCodeTitle"),
    btnCancel: t("button.cancel"),
    btnSave: t("button.save"),
    okText: t("button.ok"),
    editDiscountSuccess: t("discountCode.editDiscountSuccess"),
  ...
  const getInitialData = async (discountCodeId) => {
    getCurrency();
    getListBranches();

    let promises = [];
    promises.push(discountCodeDataService.getDiscountCodeByIdAsync(discountCodeId));
    promises.push(customerSegmentDataService.getCustomerSegmentByStoreIdAsync());
    const [discountCodeData, customerSegmentDataResponse] = await Promise.all(promises);
    setCustomerSegmentInStore(customerSegmentDataResponse);
    if (discountCodeData) {
      let data = discountCodeData?.discountCode;
      setIsAllBranches(data?.isAllBranches);
      setIsLimitNumberCouponUse(data?.isLimitNumberCouponUse);
      setStartDate(convertUtcToLocalTime(data?.startDate));
      setIsPercentDiscount(data?.isPercentDiscount);
      setDiscountCodeTypeId(data?.discountCodeTypeId);
      onChangeDiscountCodeType(data?.discountCodeTypeId);
      setIsMinimumPurchaseAmount(data?.isMinimumPurchaseAmount);
      setIsLimitOneTimeUse(data?.isLimitOneTimeUse);
      setIsAllCustomers(data?.isAllCustomers);
      if (!data?.isAllCustomers) {
        setListCustomerSegmentId(data?.customerSegmentIds);
        setTotalSegment(data?.totalSegment);
        setTotalCustomer(data?.totalCustomer);
        setShowCountCustomerSegment(true);
      }
      setIsClickCustomerSegment(false);
      ...
      let formValue = {
        discountCode: {
          ...
          code: data?.code,
          name: data?.name,
          discountCodeTypeId: data?.discountCodeTypeId,
          isPercentDiscount: data?.isPercentDiscount,
          percentNumber: data?.percentNumber,
          maximumDiscountAmount: data?.maximumDiscountAmount == 0 ? null : data?.maximumDiscountAmount,
          startDate: convertUtcToLocalTime(data?.startDate),
          startTime: convertUtcToLocalTime(data?.startDate),
          endDate: convertUtcToLocalTime(data?.endDate),
          ...
          maximumLimitCouponUse: data?.maximumLimitCouponUse,
          isLimitOneTimeUse: data?.isLimitOneTimeUse,
          platformIds: data?.platforms?.map((p) => p?.platformId),
          productCategoryIds: data?.isApplyAllCategories
            ? []
            : data?.productCategories?.map((p) => p?.productCategoryId),
          productPriceIds: data?.isApplyAllProducts ? [] : data?.productPrices?.map((p) => p?.id),
          isAllCustomers: data?.isAllCustomers,
        },
      };
  ...
  const onUpdateDiscountCode = (values) => {
    ...
    if (discountCodeStartDate > currentDateTime) {
      var req = {
        discountCode: {
          ...discountCode,
          ...
          isChangeCustomerSegment: isClickCustomerSegment,
      ...
      discountCodeDataService
        .editDiscountCodeAsync(req)
        .then((res) => {
          if (res) {
            onCancel();
            message.success(pageData.editDiscountSuccess);
          }
        })
        .catch((errs) => {
  ...
  const onCheckMaximumLimitCouponUse = (e) => {
    setIsLimitNumberCouponUse(e.target.checked);
    let formValue = form.getFieldsValue();
    let { discountCode } = formValue;
    if (e.target.checked) {
      if (!discountCode.maximumLimitCouponUse || discountCode.maximumLimitCouponUse == 0) {
        discountCode.maximumLimitCouponUse = 1;
        form.setFieldsValue(formValue);
      }
    } else {
      discountCode.maximumLimitCouponUse = 0;
      form.setFieldsValue(formValue);
    }
  };
  ...
  const onUpdateCustomerSegment = (values, customerSegmentInSore) => {
    ...
    let formValue = form.getFieldsValue();
    let { discountCode } = formValue;
    discountCode.listCustomerSegmentId = selectedCustomersSegment?.map((a) => a.id);
    form.setFieldsValue(formValue);
  };
  ...
  const onSelectAllCustomer = (isChecked) => {
    ...
    if (isChecked) {
      ...
      setIsClickCustomerSegment(true);
      onUpdateCustomerSegment([], customerSegmentInSore);
      let formValue = form.getFieldsValue();
      let { discountCode } = formValue;
  ...
  const onClickCustomerSegment = () => {
    if (!isClickCustomerSegment) {
      setIsClickCustomerSegment(true);
      onUpdateCustomerSegment(listCustomerSegmentId, customerSegmentInSore);
    }
  };
            ...
            <Row gutter={[32, 16]}>
              <Col xs={24} lg={12}>
                ...
                <Row className="w-100">
                  ...
                  <Col xs={8} lg={8} className="padding-left">
                    <Form.Item
                    ...
                    >
                      <TimePicker
                        ...
                        onSelect={(time) => {
                          ...
                          // Clear end date after select start date if endate < startdate only
                          const formValues = form.getFieldsValue();
                          if (formValues.discountCode?.endDate != null && formValues.discountCode?.endTime < time) {
                            form.setFieldsValue({
                              ...formValues,
                              discountCode: {
                                endTime: null,
              ...
              <Col xs={24} lg={12}>
                ...
                <Row className="w-100">
                  ...
                  <Col xs={10} lg={8} className="padding-left">
                    <Form.Item
          ...
          <Card className="fnb-card w-100">
            ...
            {discountCodeTypeId === PromotionType.DiscountTotal && (
              <>
                <Row className="mb-2">
                  <Checkbox
                    checked={isMinimumPurchaseAmount}
                      ...
                      rules={[
                        ...
                        () => ({
                          validator(_, value) {
                            ...
                            return Promise.reject(pageData.form.pleaseInputValue);
                          },
                        }),
                      ]}
                      className="w-100"
            ...
            <Checkbox checked={isAllCustomers} onChange={(event) => onSelectAllCustomer(event.target.checked)}>
            ...
            {!isAllCustomers ? (
              <Form.Item
                ...
                rules={[
                  {
                    required: isClickCustomerSegment,
              ...
              >
                <Select
                  ...
                  mode="multiple"
                  onChange={(e) => onUpdateCustomerSegment(e, customerSegmentInSore)}
                  ...
                  onClick={() => onClickCustomerSegment()}
                  placeholder={!showCountCustomerSegment && pageData.form.customerGroupPlaceholder}
                >
        ...
        <Row className="mt-3">
          <Card className="fnb-card w-100">
            ...
            <Row className="mb-2">
              <Checkbox
                valuePropName="checked"
                checked={isLimitNumberCouponUse}
                noStyle
                onChange={(e) => onCheckMaximumLimitCouponUse(e)}
              >
            ...
            <Row>
              <Form.Item name={["discountCode", "isLimitOneTimeUse"]}>
                <Checkbox checked={isLimitOneTimeUse} onChange={(e) => setIsLimitOneTimeUse(e.target.checked)}>
            ...
            <Row>
              <Col xs={24} lg={12}>
                ...
                <Row className="w-100">
                  ...
                  <Col xs={4} lg={6} className="padding-left">
                    <Button
                      type="primary"
