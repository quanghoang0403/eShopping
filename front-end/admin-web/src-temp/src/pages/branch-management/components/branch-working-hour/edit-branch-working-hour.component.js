...
export const EditBranchWorkingHourComponent = forwardRef((props, ref) => {
  const {
    setCurrentWorkingHoursData,
    setIsCheckedFullTime,
    setIsSelectedAllDays,
    formSelectTimeOption,
    setIsChangeForm,
  } = props;
  ...
  const [workingHoursAllDaysTempData, setWorkingHoursAllDaysTempData] = useState([]);
  const branchWorkingHours = useSelector((state) => state?.branch?.branchWorkingHours);

  useImperativeHandle(ref, () => ({
    setInitData(isFullTime, isAllDay, currentWorkingHours) {
      setIsFullTime(isFullTime);
      setIsAllDay(isAllDay);

      if (currentWorkingHours?.length > 0) {
        ///Mapping working hours here
        let workingHoursForm = currentWorkingHours?.map((item) => {
          return {
            ...item,
            openTime: moment(item.openTime, DateFormat.HH_MM),
            closeTime: moment(item.closeTime, DateFormat.HH_MM),
          };
        });

        let reduxData = null;
        if (isAllDay) {
          ///Mapping data IsAllDay
          selectTimeOptionRef?.current?.getDayOfWeek(DayOfWeekConstants.ALL_DAYS);

          setWorkingHoursAllDaysTempData(workingHoursForm);
          workingHoursForm = workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.ALL_DAYS);
          reduxData = {
            allDays: workingHoursForm,
          };
        } else {
          ///Mapping data Specific day
          onClickDayOfWeek(DayOfWeekConstants.MONDAY);

          reduxData = {
            monday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.MONDAY),
            tuesday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.TUESDAY),
            wednesday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.WEDNESDAY),
            thursday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.THURSDAY),
            friday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.FRIDAY),
            saturday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.SATURDAY),
            sunday: workingHoursForm?.filter((x) => x.dayOfWeek === DayOfWeekConstants.ALL_DAYS),
          };

          const currentActiveDays = workingHoursForm
            ?.filter((item) => item.isActive)
            ?.map((item) => (item.dayOfWeek === 0 ? DayOfWeekConstants.SUNDAY : item.dayOfWeek));

          const currentDayOfWeekSelected = [...new Set(currentActiveDays)];
        ...
        if (reduxData) {
          ...
          formSelectTimeOption.setFieldsValue(reduxData);
        }
      } else {
        setIsAllDay(true);
      }
  ...
  const formatWorkingHoursDataBeforeSave = (branchWorkingHoursData, initIsAllDay) => {
    ...
    if (currentCheckIsAllDay) {
      ...
      if (allDaysData?.length > 0) {
        const mappingWorkingHoursTempData = workingHoursAllDaysTempData?.map((item) => {
          return {
            ...item,
            dayOfWeek: item.dayOfWeek === 0 ? DayOfWeekConstants.SUNDAY : item.dayOfWeek,
          };
        });

        allDaysData?.forEach((data, index) => {
          listDayOfWeek?.forEach((day) => {
            const workingHour = {
              id: mappingWorkingHoursTempData?.filter((item) => item.dayOfWeek === day)[index]?.id,
