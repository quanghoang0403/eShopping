...
export default function DeleteBranch(props) {
  ...
  const renderRowByIndex = (purchaseOrders) => {
    let elements = [];
    for (var item = 0; item < purchaseOrders?.length; item += 1) {
      let element = (
      ...
      >
          ...
          <Col span={24}>
            {infoPurchaseOrder.isOpenPurchaseOrder ? (
              <>
                ...
                ></div>
                <div className="table-notification">{renderRowByIndex(infoPurchaseOrder?.purchaseOrders)}</div>
              </>
            ) : (
              <p
                dangerouslySetInnerHTML={{
        ...
        {infoPurchaseOrder.isOpenPurchaseOrder ? (
          ...
          <Row className="modal-footer">
            ...
            <Button danger onClick={() => onDelete(infoPurchaseOrder?.branch?.id, infoPurchaseOrder?.branch?.name)}>
              {pageData.delete}
            </Button>
          </Row>
        )}
