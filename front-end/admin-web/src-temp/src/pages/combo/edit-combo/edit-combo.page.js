...
export default function EditCompoPage(props) {
                  ...
                  <div className="option-group-item-responsive">
                    <Row align="middle">
                      <Col span={24} className="item-responsive-product-name text-bold">
                        <Text className="item-option-group-product">{optionData?.text}</Text>
                      </Col>
                    </Row>
                    <Row align="middle">
                      <Col span={24} className="text-normal">
  ...
  const maxComboSellingPriceValidatorForFixedPrice = (rule, value, callback) => {
    if (value && value > maxPrice) {
      const validateMessage = pageData.price.sellingPriceValidateMessage + formatTextNumber(maxPrice + 1);
  ...
  const maxComboSellingPriceValidatorForSpecificPrice = (rule, value, callback, originalPrice) => {
    if (value && value >= originalPrice) {
      const validateMessage = pageData.price.sellingPriceValidateMessage + formatTextNumber(originalPrice);
      callback(validateMessage);
    }
    callback();
  };
      ...
      <>
        ...
        <Form.Item
          ...
          rules={[
            ...
            { validator: maxComboSellingPriceValidatorForFixedPrice },
          ]}
        >
          <InputNumber
            onChange={(value) => {
  ...
  const onChangeProduct = async (listSelectedItemName, options, index) => {
      ...
      <>
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: pageData.price.sellingPriceValidateMessage + formatTextNumber(totalOriginalPriceOfSpecificCombo),
            },
