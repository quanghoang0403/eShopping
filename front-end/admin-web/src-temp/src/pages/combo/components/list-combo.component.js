...
import { Col, Modal, Popover, Row, Typography, message } from "antd";
...
import { formatDate, formatTextNumber, getCurrency, getThumbnailUrl, hasPermission } from "utils/helpers";
    ...
    const columns = [
          ...
          return (
            <Row align="middle">
              <Paragraph placement="top" ellipsis={{ tooltip: record?.name }} color="#50429B">
                <ThumbnailCombo src={getThumbnailUrl(record?.thumbnail, "mobile")} />
                            ...
                            <Col span={24} align="left">
                              <div className="product-price-item">
                                <Paragraph
                                  ...
                                  color="#50429B"
                                >
                                  <span className="product-price-item__name">
      ...
      },
      {
        ...
        render: (_, record) => (
          <a
            onClick={() => {
              if (record.totalBranch > 0) {
                setShowBranchModel(true);
              }
              initDataBranch(record);
            }}
          >
            {record.totalBranch}
          </a>
            ...
            case ComboStatus.Schedule:
              return (
                <div className="status-scheduled">
                  <span>{t("combo.status.scheduled")}</span>
                </div>
              );
            case ComboStatus.Active:
              return (
                <div className="combo-status-active">
                  <span>{t("combo.status.active")}</span>
                </div>
              );
            default:
              return (
                <div className="status-finished">
                  <span>{t("combo.status.finished")}</span>
                </div>
              );
          }
        },
            ...
            <div className="action-column">
              ...
              {hasPermission(PermissionKeys.STOP_COMBO) && record.statusId === ComboStatus.Active && (
                <DeleteConfirmComponent
                  ...
                  content={t(pageData.confirmStopCombo, { name: record?.name })}
