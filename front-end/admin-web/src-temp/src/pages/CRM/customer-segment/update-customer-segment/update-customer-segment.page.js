...
import "./update-customer-segment.scss";
...
export default function UpdateCustomerSegment(props) {
        ...
        if (response) {
          ...
          let formValues = {
            ...
            conditions: customerSegment.customerSegmentConditions?.map((condition) => ({
              ...
              ageConditionId: condition.ageConditionId,
              age: condition.age,
              orderDataId: condition.orderDataId ?? orderDataEnum.totalCompletedOrders,
              orderConditionId: condition.orderConditionId ?? orderConditionEnum.isEqual,
              orderEndTime: condition.orderEndTime ? moment(condition.orderEndTime) : null,
              orderStartTime: condition.orderStartTime ? moment(condition.orderStartTime) : null,
              orderTime: {
                startDate: condition.orderStartTime ? condition.orderStartTime : null,
                endDate: condition.orderEndTime ? condition.orderEndTime : null,
              },
              orderTypeFilterTime: condition.orderTypeFilterTime ?? OptionDateTime.allTime,
              purchasedAmount: condition.purchasedAmount,
              days: condition.days,
  ...
  const deleteCondition = (index) => {
    const { id, name, isAllMatch, conditions } = formData;
      ...
      <>
        <Row key={index}>
          <Col
            ...
            lg={5}
            style={
              isMobileMode ? { maxWidth: "266px", marginLeft: "40px" } : { paddingRight: "16px", marginLeft: "40px" }
            }
          >
            ...
            <Form.Item name={["conditions", index, "objectiveId"]}>
              <Select
                ...
                defaultValue={condition.objectiveId ?? objectiveEnum.customerData}
          ...
          <Col sm={24} xs={24} lg={15}>
            <Row>
              ...
              <Col
                ...
                lg={8}
                style={
                  isMobileMode ? { maxWidth: "266px", marginLeft: "40px" } : { paddingRight: "6px", marginLeft: "0px" }
                }
              >
                {condition?.orderDataId === orderDataEnum.totalCompletedOrders && (
                  <div className={`${!isMobileMode && "ml-2"}`}>
                    <h4 className="fnb-form-label">{pageData.condition.orderNumber.label}</h4>
                    <Form.Item
                ...
                {condition?.orderDataId === orderDataEnum.totalPurchasedAmount && (
                  <div className={`${!isMobileMode && "ml-2"}`}>
                    <h4 className="fnb-form-label">{pageData.condition.totalPurchaseAmount.label}</h4>
                    <Form.Item
                ...
                {condition?.orderDataId === orderDataEnum.days && (
                  <div className={`${!isMobileMode && "ml-2"}`}>
                    <h4 className="fnb-form-label">{pageData.condition.days}</h4>
                    <Form.Item
