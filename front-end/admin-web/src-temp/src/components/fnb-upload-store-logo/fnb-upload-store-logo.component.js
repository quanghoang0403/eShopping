...
import { setInformationPublishStore, setStoreLogo } from "store/modules/session/session.actions";
...
export const FnbUploadStoreLogoComponent = forwardRef((props, ref) => {
  ...
  const onUploadImage = async (imageList) => {
    ...
    dispatch(setInformationPublishStore({ ...infoStore, logo: updateStoreLogoResponse }));
    setImages(imageList);
