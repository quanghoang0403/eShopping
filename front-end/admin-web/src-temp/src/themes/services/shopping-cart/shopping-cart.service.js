...
import { checkNonEmptyArray, getStoreConfig } from "../../utils/helpers";
...
const verifyAndUpdateCart = async (data) => {
  ...
  const verifyCartRequest = {
    ...
    discountCodes: checkNonEmptyArray(reduxData?.discountCodes) ? reduxData?.discountCodes : [],
  ...
  await verifyCartAsync(verifyCartRequest)
    .then((response) => {
      if (response?.status === HttpStatusCode.Ok) {
        ...
        if (
          checkNonEmptyArray(reduxData?.appliedDiscountCodes) &&
