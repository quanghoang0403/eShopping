...
import { CloseCircleIcon, LocationGoogleDropdownIcon, LocationGoogleIcon } from "../../../assets/icons.constants";
...
export const PlacesAutocompleteComponent = forwardRef((props, ref) => {
  ...
  const { onSelectLocation, onEmptyLocation, placeholder, initLocation, initAddress, groupColorConfig, maxLength } = props;
  ...
  return (
    <div ref={refClickOutside}>
      <FnbInput
        ...
        prefix={<LocationGoogleIcon />}
        maxLength={maxLength}
      />
      {/* We can use the "status" to decide whether we should display the dropdown or not */}
      {status === "OK" && <div className="delivery-address-popover">{renderSuggestions()}</div>}
    </div>
  );
});
