...
const ProductListScrollSpyContext = createContext(
    {categories: [], loading: false, sources: undefined, tab: ''}
);
...
const ProductListScrollSpyProvider = ({children, header = undefined}) => {
    ...
    const handleUpdateSetLoading = (_key, isAdd = true) => {
        if (isAdd) {
            if (listLoadings.indexOf(_key) === -1)
                setListLoadings(listLoadings.concat([_key]));
        } else {
    ...
    }, [categories, currency, productStyles, handleSetStoreCurrency, listLoadings, handleSetProductStyles, tab, handleSetScrollContainerRef, sources, loading, setLoading, setCategories, setSources, handlePressTab, handleSetTabActive, handleLoadMoreCategory]);

    return <ProductListScrollSpyContext.Provider value={getValue()}>
        {header}
        {
