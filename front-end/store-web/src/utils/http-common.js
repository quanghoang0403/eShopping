...
import { PACKAGE_EXPIRED, PLATFORM_ID } from "../constants/config.constants";
import { env } from "../env";
...
export const createHttp = () => {
  ...
  const baseApi = storeConfig?.apiUrl ?? `${env.REACT_APP_ROOT_DOMAIN}/api`;
  var date = new Date();
  ...
  http.interceptors.response.use(
    ...
    (error) => {
      if (error?.response?.status === HttpStatusCode.BadRequest) {
        if (error?.response?.data?.errorType === PACKAGE_EXPIRED) {
          store?.dispatch(setPackageExpiredInfo(error?.response?.data));
        }
