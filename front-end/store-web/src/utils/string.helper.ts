...
export const getRandomStarRating = (): string => {
  return (Math.random() * (5 - 3) + 3).toFixed(1) // Random star rating between 3.0 and 5.0
}

export const getRandomReviews = (): number => {
  return Math.floor(Math.random() * 10) + 1 // Random review count between 1 and 100
}
