...
const GridCategoryList: FC<GridCategoryListProps> = ({
  ...
  const renderCard = (item: ExploreType) => {
    switch (boxCard) {
      ...
      case 'box4':
        return <CardCategory4 bgSVG={item.svgBg} featuredImage={item.image} key={item.id} color={item.color} {...item} />
      case 'box6':
        return <CardCategory6 bgSVG={item.svgBg} featuredImage={item.image} key={item.id} color={item.color} {...item} />
