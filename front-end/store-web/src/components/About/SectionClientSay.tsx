...
import clientSayMain from '@/assets/images/clientSay/clientSayMain.png'
import quotationImg from '@/assets/images/clientSay/quotation.png'
import quotationImg2 from '@/assets/images/clientSay/quotation2.png'
...
const SectionClientSay: FC<SectionClientSayProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`nc-SectionClientSay relative flow-root ${className} `} data-nc-id="SectionClientSay">
      ...
      <div className="relative md:mb-16 max-w-2xl mx-auto">
        ...
        <div ref={sliderRef} className={`mt-12 lg:mt-16 relative ${isShow ? '' : 'invisible'}`}>
          <Image className="opacity-50 md:opacity-100 absolute -mr-16 lg:mr-3 right-full top-1" src={quotationImg} alt="" />
          <Image className="opacity-50 md:opacity-100 absolute -ml-16 lg:ml-3 left-full top-1" src={quotationImg2} alt="" />
          ...
          <div className="mt-10 glide__bullets flex items-center justify-center" data-glide-el="controls[nav]">
            {DEMO_DATA.map((item, index) => (
              <button key={item.id} className="glide__bullet w-2 h-2 rounded-full bg-neutral-300 mx-1 focus:outline-none" data-glide-dir={`=${index}`}></button>
            ))}
          </div>
        </div>
      </div>
